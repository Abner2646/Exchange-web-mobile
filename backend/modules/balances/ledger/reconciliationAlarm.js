...
async function runReconciliationCheck({ reconcileInternal, reconcileExternal, logger = console } = {}) {
  ...
  const [internoR, externoR] = await Promise.allSettled([reconcileInternal(), reconcileExternal()]);
  ...
  if (!interno.ok) {
    logger.error('[reconciliation] ALARMA: la proyección no coincide con la suma de movimientos', {
      discrepancies: interno.discrepancies, error: interno.error,
    });
  }
  if (!externo.ok) {
    logger.error('[reconciliation] ALARMA: el libro no cierra en cero por cripto', {
      byCrypto: externo.byCrypto, error: externo.error,
    });
  }
