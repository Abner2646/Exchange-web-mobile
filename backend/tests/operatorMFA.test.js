...
// (`twoFactorEnabled`), leído autoritativo de la DB (no del token, que podría
...
test('operador SIN 2FA → 403 OPERATOR_MFA_REQUIRED', async () => {
  User.findByPk.mockResolvedValue({ twoFactorEnabled: false });
  const next = jest.fn();
...
test('operador CON 2FA → next() sin error', async () => {
  User.findByPk.mockResolvedValue({ twoFactorEnabled: true });
  const next = jest.fn();
