...
describe('cambio de email — flujo feliz', () => {
  test('solicitar → confirmar: email actualizado + verificado, cooldown seteado, notifica al viejo', async () => {
    ...
    const req1 = await request(app).post('/api/usuario/me/email-change').set(auth)
      .send({ nuevoEmail: 'nuevo@test.local', passwordActual: 'password123' });
    expect(req1.status).toBe(200);
    const codigo = codeFor('nuevo@test.local');
    expect(codigo).toBeTruthy();

    const req2 = await request(app).post('/api/usuario/me/email-change/confirm').set(auth)
      .send({ codigo });
    expect(req2.status).toBe(200);

    const u = await Usuario.findOne({ where: { email: 'nuevo@test.local' } });
    expect(u).not.toBeNull();
    expect(u.emailVerificado).toBe(true);
    expect(u.emailPendiente).toBeNull();
    expect(u.cooldownRetiroHasta).toBeTruthy();
    expect(new Date(u.cooldownRetiroHasta).getTime()).toBeGreaterThan(Date.now());
...
describe('cambio de email — rechazos', () => {
  test('contraseña actual incorrecta → 401 EMAIL_CHANGE_INVALID', async () => {
    ...
    const res = await request(app).post('/api/usuario/me/email-change').set(auth)
  ...
  test('confirmar con código incorrecto → 400', async () => {
    ...
    await request(app).post('/api/usuario/me/email-change').set(auth)
      .send({ nuevoEmail: 'nuevoe@test.local', passwordActual: 'password123' });
    const res = await request(app).post('/api/usuario/me/email-change/confirm').set(auth)
      .send({ codigo: '000000' });
    expect(res.status).toBe(400);
    expect(res.body.error.code).toBe('EMAIL_CHANGE_INVALID');
  });
...
describe('cambio de email — enforcement del cooldown de retiros', () => {
  test('tras confirmar el cambio, un retiro se rechaza con 403 WITHDRAWAL_COOLDOWN', async () => {
    ...
    await request(app).post('/api/usuario/me/email-change/confirm').set(auth)
