// Covers AUDITORIA_BACKEND.md Críticos #5: UserBalance.blockBalance did
...
describe('UserBalance.blockBalance under concurrency (real Postgres)', () => {
  test('two concurrent 80-blocks over a 100 balance: only one may pass', async () => {
    const results = await Promise.allSettled([
      UserBalance.blockBalance(user.id, cripto.id, 80),
      UserBalance.blockBalance(user.id, cripto.id, 80),
    ]);
    ...
    const finalBalance = await UserBalance.getByUserAndCrypto(user.id, cripto.id);
    expect(finalBalance.availableBalance).toBe('20.00000000');
    expect(finalBalance.blockedBalance).toBe('80.00000000');
  }, 15000);

  test('ten concurrent 15-blocks over a 100 balance: at most 6 pass, never negative', async () => {
    const results = await Promise.allSettled(
      Array.from({ length: 10 }, () => UserBalance.blockBalance(user.id, cripto.id, 15))
    );
    ...
    const finalBalance = await UserBalance.getByUserAndCrypto(user.id, cripto.id);
    expect(parseFloat(finalBalance.availableBalance)).toBeGreaterThanOrEqual(0);
    expect(parseFloat(finalBalance.blockedBalance)).toBe(fulfilled.length * 15);
  }, 15000);
});
