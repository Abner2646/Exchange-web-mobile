...
async function seedPendingWithdrawal(user, bnb, amount) {
  return BlockchainTransaction.createWithdrawal({
    userId: user.id, cryptoId: bnb.id, amount, destinationAddress: '0xrecipient0000000000000000000000000000dead',
  });
}
...
describe('BNB native withdrawal — processPendingWithdrawals (fake chain)', () => {
  ...
  test('BEP20 token withdrawal goes through sendTokenTransfer', async () => {
    ...
    const w = await BlockchainTransaction.createWithdrawal({
      userId: user.id, cryptoId: busd.id, amount: '100', destinationAddress: '0xrecipient0000000000000000000000000000dead',
    });
