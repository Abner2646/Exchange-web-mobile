...
describe('POST /api/usuario/register', () => {
  test('creates an active-unverified user, returns a token, emails exactly one verification code', async () => {
    ...
    const user = await User.findOne({ where: { email: 'newuser@test.local' } });
    expect(user).not.toBeNull();
    expect(user.active).toBe(true);          // characterize entity default
    expect(user.emailVerified).toBe(false);

    // exactly one verification email, to this address, carrying a non-empty code
    const verifications = h.fake.sent.filter((s) => s.type === 'verificacion');
...
describe('POST /api/usuario/register — rejections', () => {
  test('rejects a duplicate email/username', async () => {
    ...
    const first = await request(app).post('/api/user/register').send(body);
    expect(first.status).toBe(201);

    const second = await request(app).post('/api/user/register').send(body);
