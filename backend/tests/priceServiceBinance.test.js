...
// contaminaba con float binario antes de guardarlos en SwapPair (DECIMAL).
// El price inverso (1/price) se calculaba con división float. Ahora: strings
...
describe('priceService.updateFromBinance — precios exactos como string', () => {
  test('price directo: preserva el string de Binance (no float)', async () => {
    ...
    const par = { id: 'p1', baseCrypto: { symbol: 'BTC' }, quoteCrypto: { symbol: 'USDT' } };
    await priceService.updateFromBinance([par]);

    expect(spy).toHaveBeenCalledWith('p1', expect.objectContaining({
      currentPrice: '43250.123456789012345',
      volume24h: '100.5',
    }));
    spy.mockRestore();
  });

  test('price inverso: 1 / 0.00002 = 50000 exacto (float da 49999.99999999999)', async () => {
    axios.get.mockResolvedValue({
      data: [{
    ...
    const par = { id: 'p2', baseCrypto: { symbol: 'BTC' }, quoteCrypto: { symbol: 'USDT' } };
    await priceService.updateFromBinance([par]);

    expect(spy).toHaveBeenCalledWith('p2', expect.objectContaining({ currentPrice: '50000' }));
    spy.mockRestore();
  });
});
