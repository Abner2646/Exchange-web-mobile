...
class CryptoService {
  ...
  async getActiveCryptos() {
    const response = await api.get('/criptomonedas/activas');
  ...
  async getCryptoById(id) {
    try {
      const response = await api.get(`/criptomonedas/${id}`);
  ...
  async getCryptoBySymbol(symbol) {
    try {
      const response = await api.get(`/criptomonedas/symbol/${symbol}`);
      return response.data;
    } catch (error) {
  ...
  async getPrice(from, to) {
    try {
      const response = await api.get(`/exchange/price/${from}/${to}`);
