...
| 500.12 | Multi-factor authentication | 🟡 | Email-code 2FA for user login — baseline §1, `tests/integration/auth2FA...`. **Operator MFA enforced (§4.9): `requireOperatorMFA` requires an operator (admin/super_admin) to have 2FA enabled before privileged `/system/*` money-trigger actions** (`middleware/operatorMFA.middleware.js`, `tests/operatorMFA.test.js`) | Extend the guard to all privileged operator endpoints; per-action step-up re-auth + separate operator identity realm (Fase 5 / Cognito) |
