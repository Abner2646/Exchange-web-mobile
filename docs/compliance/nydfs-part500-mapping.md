...
| 500.5 | Vulnerability mgmt & penetration testing (annual) | 🟡 | **Dependency scanning in CI (§4.6): `npm audit` hard gate at high/critical + Dependabot weekly PRs** (`.github/workflows/ci.yml`, `.github/dependabot.yml`); tree clean at high/critical after the gate caught and forced a real fix (nodemailer 6→10, credential-interception/SSRF advisories). Automated tests + mutation testing on money modules; `/security-review` available. No formal pentest yet | Documented pentest pass pre-mainnet; moderate/low advisories via Dependabot |
...
| 500.12 | Multi-factor authentication | 🟡 | Email-code 2FA for user login — baseline §1, `tests/integration/auth2FA...`. **Operator MFA enforced (§4.9): `requireOperatorMFA` requires an operator to have 2FA enabled before privileged actions** — the `/system/*` money-triggers, admin balance adjustments (updateBalance/block/unblock/transfer), master-wallet creation, setup initialize, and P2P force-status (`middleware/operatorMFA.middleware.js`, `tests/operatorMFA.test.js`) | Per-action step-up re-auth + separate operator identity realm (Fase 5 / Cognito) |
