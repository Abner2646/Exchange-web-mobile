...
Remaining: prerequisite **(1)** — verify the current Part 500 / FinCEN control text against sources, then write the control rows citing the baseline as evidence. Until (1) is done, this file stays a scaffold on purpose — populating control rows from memory would defeat its audit-grade point.
